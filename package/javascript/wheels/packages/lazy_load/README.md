...
使用`<img>`标签的时候，先不把图片路径设置在`src`属性上，而是设置在`data-src`的自定义属性上，然后通过监听该元素是否显示在窗口，如果显示了就将`data-src`的值赋给`src`属性。

```html
<img data-src="https://img.cc0.cn/pixabay/2019102204441869402.jpg!cc0.cn.jpg">
```

## 滚动监听
滚动监听，使用`getBoundingClientRect()`获取各个元素的位置，然后与窗口高度进行比较，如果进行渲染。
...
`IntersectionObserver`接口用来观察一个元素是否在视窗可见。

因此可用其对每个图片元素进行监听，判断是否在视图可见，并进行渲染。

该方法必须给`img`设置最小宽高，否则初始化就会加载全部图片，而失去了懒加载的功能。
