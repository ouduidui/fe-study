...
onMounted(() => {
  const containerWidth = barChart.value.parentElement.offsetWidth;
  ...
  let x = d3
    ...
    .rangeRound([0, width])
    .padding(0.1)
    .domain(data.map(d => d.letter)) // 设置x轴

  let y = d3
    .scaleLinear()
    .rangeRound([height, 0])
    .domain([0, d3.max(data, function(d) {return d.frequency})]) // 设置y轴
...
<template>
  <h2>柱状图</h2>
  <div class="bar-chart">
    <svg ref="barChart" id="bar-chart"></svg>
  </div>
</template>
